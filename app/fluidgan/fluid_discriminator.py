...
class FluidDiscriminator(tf.keras.Model):
    ...
    def loss(self, logits_real, logits_fake):
        D_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(logits_fake), logits=logits_fake))
        D_loss += tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(logits_real), logits=logits_real))
        return D_loss

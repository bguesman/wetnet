...
def train(model, discrim, train_low, train_hi):
    """
    ...
    for i in range(train_low.shape[0]):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            fake_logits = discrim(upsampled)
            real_logits = discrim(labels)
            ae_loss = model.loss(upsampled, labels, fake_logits)
            discrim_loss = discrim.loss(real_logits, fake_logits)
        # Optimize.
        gradients = tape.gradient(ae_loss, model.trainable_variables)
        model.optimizer.apply_gradients(zip(gradients, model.trainable_variables))
        gradients = tape.gradient(discrim_loss, discrim.trainable_variables)
...
def test(model, test_low, test_hi):
    ...
    for i in range(test_low.shape[0]):
        ...
        loss = model.old_loss(upsampled, batch_labels)
        # Accumulate loss.
...
def main():
    ...
    discrim = FluidDiscriminator(test_hi.shape[:])
    # Train and Test Model.
    start = time.time()
    epochs = 1
    for i in range(epochs):
        train(model, discrim, train_low, train_hi)
    end = time.time()
